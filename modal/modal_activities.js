class modal_Activities {
  ...
  make(actlists) {
    ...
    actlists.forEach((act, idx) => {
      ...
      head.innerHTML =
        act.title +
        ` <button type="button" class="btn-sm btn-light ml-1 pl-2 pr-2 pt-0 pb-0" onclick="cMapMaker.shareURL('${act.id}')">
            <i class="fas fa-clone"></i></button>`;
      head.setAttribute("id", act.id.replace("/", ""));
      let edit = Conf.etc.editMode
        ? `[<a href="javascript:modal_activities.edit({id:'${
            act.id
          }',form:'${newmode}'})">${glot.get("act_edit")}</a>]`
        : "";
      let chtml = Conf.etc.editMode
        ? `<div class="float-right">${glot.get(
            "update"
          )} ${updated}${edit}</div><br>`
        : "";
      // // 編集可能画面にする場合は以下 chtmlの行を有効化する
      ...
      // // Read Only の場合は以下 chtmlの行を有効化する
      // let chtml = `<div class="float-right">${glot.get("update")} ${updated}</div>`;
      chtml +=
        glot.get("share_link") +
        `<button type="button" class="btn-sm btn-light ml-1 pl-2 pr-2 pt-0 pb-0" onclick="cMapmaker.url_share('${act.id}')">
      ...
      switch (newmode) {
        case "libc":
          ...
          chtml += `<strong>${glot.get("libc_title")}</strong><br>${act_ymd} ${
            act.title
          }<br><br>`;
          chtml +=
            "<strong>" +
            glot.get("libc_agency") +
            "</strong><br>" +
            act.agency +
            "<br><br>";
          chtml +=
            "<strong>" +
            glot.get("libc_authority") +
            "</strong><br>" +
            act.authority.replace(/\r?\n/g, "<br>") +
            "<br><br>";
          chtml +=
            "<strong>" +
            glot.get("libc_area") +
        ...
        default: // event
          Object.keys(form).forEach((key) => {
            if (form[key].viewHidden !== true) {
              chtml += `<div class='row'>`;
              let gdata =
                act[form[key].gsheet] == undefined
                  ? ""
                  : String(act[form[key].gsheet]);
              ...
              switch (form[key].type) {
                ...
                case "quiz_textarea":
                  chtml += `<div class='col-12'>${glot.get(
                    form[key].glot
                  )}</div><div class='col-12'>${gdata.replace(
                    /\r?\n/g,
                    "<br>"
                  )}</div>`;
                  break;
  ...
  make_activity_list(actlists) {
    if (actlists.length > 1) {
      ...
      for (let act of actlists) {
        ...
        html += `<li><span class="pointer" onclick="document.getElementById('${act.id.replace(
          "/",
          ""
        )}').scrollIntoView({behavior: 'smooth'})">${act.title}<span></li>`;
      }
      return html + "</ul>";
    } else {
      return "";
  ...
  edit(params) {
    ...
    let data =
      params.id === void 0
        ? { osmid: cMapMaker.open_osmid }
        : poiCont.get_actid(params.id);
    ...
    Object.keys(act[params.form].form).forEach((key) => {
      ...
      switch (form.type) {
        ...
        case "comment":
          html += `<div class='col-2 p-1'>${glot.get(`${form.glot}`)}</div>`;
          html += `<div class='col-10 p-1'><h5>${glot.get(
            `${form.glot}`
          )}</h5></div>`;
          break;
        case "attention":
    ...
    html += `</div></div>`;
    html += `<input type="hidden" id="act_id" value="${
      params.id === void 0 ? "" : params.id
    }"></input>`;
        ...
        if (!modal_activities.busy && userid !== "" && passwd !== "") {
          ...
          Object.keys(act[params.form].form).forEach((key) => {
            ...
            if (field.gsheet !== "" && field.gsheet !== undefined)
              senddata[field.gsheet] = document.getElementById(
                "act_" + key
              ).value;
          });
          gSheet
            ...
            .then((e) => {
              ...
              if (e.status.indexOf("ok") > -1) {
                ...
                gSheet.get(Conf.google.AppScript).then((jsonp) => {
                  ...
                  let targets =
                    Conf.listTable.target == "targets"
                      ? [listTable.getSelCategory()]
                      : ["-"];
  ...
  viewImage(e) {
    ...
    document.getElementById("full_screen_image").style["background-image"] =
      "url('" + src_org + "')";
  }

  closeImage() {
    document
      .getElementById("full_screen_image")
      .classList.remove("isFullScreen");
  }
}
