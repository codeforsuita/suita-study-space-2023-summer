...
class ListTable {
	...
	filterCategory(categorys) {
		...
		if (this.list.length > 0) {
			this.flist = categorys !== "-" ? this.#filter(categorys, this.list[0].length - 2) : this.list;	// 最後-2列を指定してフィルタリング
			...
			switch (target) {
				...
				case "tags":		// タグからカテゴリを表示
					...
					pois.filter(Boolean).sort().map(poi => {
						let already = winCont.select_add(`list_category`, poi[1], target + "," + poi[poi.length - 2]);	// target + 元タグ
						if (!already) this.categorys.push(target + "," + poi[poi.length - 2]);
					});
