...
class PoiCont {
	...
	setActlnglat() {								// set Act LngLat by OSMID
		poiCont.adata.forEach((act) => { act.lnglat = poiCont.get_osmid(act.osmid).lnglat; });
	}
	...
	get_pois(targets) {		// 指定したtargetsのpoisを返す
		let tars = [], lls = [];
	...
	setPoi(target, actonly, flist) {
		...
		const checkMarker = function (poi, LL, zoomlv) {
			...
			if (geoCont.checkInner(poi.lnglat, LL)) {
				...
				let tg = actlists.length > 0 ? Conf.google.targetName : poi.targets[0];									// activity or target指定
